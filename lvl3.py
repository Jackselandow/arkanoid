...
def run_lvl():
    ...
    level3_text = basics.RETROBLAZE_FONT.render('l e v e l 3', False, basics.GREEN)
    level3_rect = level3_text.get_rect(center=(basics.WIN_RECT.right / 2, upper_menu.bottom / 2))

    while True:
        key_pressed = pygame.key.get_pressed()
        bricks_left_text = basics.PIXEBOY_FONT.render(('BRICKS LEFT: ' + str(len(basics.LEVEL.bricks) - 13)), False, basics.WHITE)
        ...
        if len(basics.LEVEL.bricks) - 13 == 0:
        ...
        basics.WIN.blit(level3_text, level3_rect)
...
def detect_lvl_start(key_pressed):
    if key_pressed[pygame.K_SPACE] and basics.LEVEL.level_started is False:
        ...
        basics.LVL3_MUSIC.play(loops=-1)
...
def lvl3_main():
    ...
    bg = pygame.transform.scale(pygame.image.load('backgrounds/lvl3_bg.png'), (basics.WIN_RECT.right, basics.WIN_RECT.bottom - upper_menu.height))
